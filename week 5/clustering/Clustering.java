...
public class Clustering {
	...
	public static void main(String[] args) {
		...
		System.out.printf("%.12f%n",clustering(x, y, k));
	}
	...
	private static double clustering(int[] x, int[] y, int k) {
		...
		PriorityQueue<Edge> edges = new PriorityQueue<>((e1, e2) -> e1.weight < e2.weight ? -1 : 1);
		for (int i = 0; i < n; i++) {
			for (int j = i + 1; j < n; j++) {
		...
		while(!edges.isEmpty()){
			...
			if(unionNum > n - k) return currentEdge.weight;
		}
		return -1;
	}
